...
class History:
    ...
    xentropy: list = dataclasses.field(default_factory=list)
    # The target objective for each population member at each iteration.
    target: list = dataclasses.field(default_factory=list)
    ...
    runtime: list = dataclasses.field(default_factory=list)
...
def epo(
    ...
    if x_penalty_min is None or x_penalty_max is None:
        X = torch.zeros(population_size, device=model_device(model))
    else:
        X = torch.exp(
            torch.linspace(
    ...
    if history is not None:
        ...
        history = History()
        input_ids = initial_ids.to(model_device(model))
    ...
    allowed_mask[allowed_ids.detach().cpu()] = True
...
def gcg(
    ...
    iters: int = 1000,
    batch_size: int = 8,
    explore_per_iter: int | None = None,
    topk: int = 32,
    ...
    return epo(
        ...
        population_size=1,
        iters=iters,
        explore_per_pop=explore_per_iter or batch_size,
...
def token_grads(
    ...
    loss = torch.empty(input_ids.shape[0], device=model_device(model))
    xentropy = torch.empty(input_ids.shape[0], device=model_device(model))
    target = torch.empty(input_ids.shape[0], device=model_device(model))
    final_token = torch.empty(
        input_ids.shape[0], device=model_device(model), dtype=torch.long)
    extra = {}
    ...
    with torch.enable_grad():
        ...
        for i in range(0, input_ids.shape[0], batch_size):
            ...
            if hasattr(cache_run, "iter_context_results"):
                this_target = torch.zeros(
                    imax - i,
                    device=one_hot.device,
                    dtype=torch.float32,
                )
                this_xentropy = torch.zeros_like(this_target)
                context_extra: dict[str, torch.Tensor] = {}
                context_count = int(cache_run.context_count)
                for cache in cache_run.iter_context_results(
                    candidate_distribution=one_hot,
                ):
                    context_xentropy = cache["xentropy"]
                    minimize = getattr(cache_run, "minimize", False)
                    sign = 1.0 if minimize else -1.0
                    context_loss = (
                        sign * cache["target"]
                        + context_xentropy * x_penalty[i:imax]
                    ) / context_count
                    context_loss.sum().backward()
                    this_target += cache["target"].detach().float() / context_count
                    this_xentropy += (
                        context_xentropy.detach().float() / context_count
                    )
                    for key, value in cache.items():
                        if key not in {"target", "logits", "xentropy"}:
                            contribution = value.detach() / context_count
                            context_extra[key] = (
                                contribution
                                if key not in context_extra
                                else context_extra[key] + contribution
                            )
                this_loss = (
                    (1.0 if getattr(cache_run, "minimize", False) else -1.0)
                    * this_target
                    + this_xentropy * x_penalty[i:imax]
                )
                cache = {
                    "target": this_target,
                    "xentropy": this_xentropy,
                    **context_extra,
                }
            else:
                cache_kwargs = {
                ...
                if getattr(cache_run, "accepts_candidate_distribution", False):
                    cache_kwargs["candidate_distribution"] = one_hot
                cache = cache_run(**cache_kwargs)

                if "xentropy" in cache:
                    this_xentropy = cache["xentropy"]
                else:
                    this_xentropy = (
                        -(
                            torch.log_softmax(cache["logits"][:, :-1], dim=-1)
                            * one_hot[:, 1:]
                        )
                        .sum(dim=-1)
                        .mean(dim=-1)
...
def evaluate_fitness(
    ...
    for i in range(0, input_ids.shape[0], batch_size):
        ...
        xentropy[i:imax] = (
            mini_batch["xentropy"]
            if "xentropy" in mini_batch
            else calc_xentropy(mini_batch["logits"], input_ids[i:imax])
        )
...
def pareto_callback(cache_run, model, tokenizer, x_penalty_min, x_penalty_max):
    def f(i, state, last_runtime, history, final=False):
        if last_runtime is not None:
            print(f"runtime: {last_runtime:.2f} seconds")
        ...
        for penalty_idx in range(len(Xvs)):
            idx = idxs[penalty_idx]
            if idx == last_idx:
                continue
            text = tokenizer.decode(state.ids[idx])
            last_token = tokenizer.decode(state.final_token[idx])
            print(
                f"penalty={Xvs[penalty_idx]:.2f} xentropy={state.xentropy[idx]:.2f} target={state.target[idx]:.2f} {text + '[' + last_token + ']'!r}"
            )
            last_idx = idx
