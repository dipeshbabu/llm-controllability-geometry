...
from llm_controllability.optimization.epo import add_fwd_hooks
...
def does_retokenize(model, tokenizer, input_ids):
    good = torch.empty(input_ids.shape[0], dtype=bool).to(model_device(model))
    input_strs = tokenizer.batch_decode(input_ids)
    for i, s in enumerate(input_strs):
        retokenized = tokenizer.encode(s, return_tensors="pt").to(model_device(model))
    ...
    def run(input_ids=None, inputs_embeds=None):
        if input_ids is not None:
            if check_retokenization:
                ...
                good = torch.ones(
                    input_ids.shape[0], dtype=bool).to(model_device(model))
            ...
            good = torch.ones(
                inputs_embeds.shape[0], dtype=bool).to(model_device(model))
