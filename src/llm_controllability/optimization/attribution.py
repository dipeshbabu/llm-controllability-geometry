...
_DEFAULT_RESAMPLE_RANKS = range(64)
...
def viz(
    ...
    for i, token in enumerate(tokens):
        ...
        tooltip = f"""
        Worst: <tt>{prep_token(worst_token)!r}</tt>, {worst_target:.3f}<br>
        Top-3: (<tt>{prep_token(top3_tokens[0])!r}</tt>, {top3_target[0]:.3f}),
            (<tt>{prep_token(top3_tokens[1])!r}</tt>, {top3_target[1]:.3f}),
            (<tt>{prep_token(top3_tokens[2])!r}</tt>, {top3_target[2]:.3f})
        """
...
def resample(
    ...
    resamplings[
        torch.arange(seq_len, device=model_device(model))[None, :],
        :,
        torch.arange(seq_len, device=model_device(model))[None, :],
    ] = topk
    ...
    for i in range(0, resamplings_flat.shape[0], batch_size):
        ...
        targets[i:end_i] = cache_run(resamplings_flat[i:end_i].to(model_device(model)))[
            "target"
        ]
