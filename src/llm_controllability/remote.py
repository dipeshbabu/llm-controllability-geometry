...
from llm_controllability.optimization.epo import epo
...
# Treat Colab and local execution as no-Modal-at-import-time environments.
...
if _MODAL_AVAILABLE and not _IS_LOCAL_LIKE:
    ...
    try:
        gpu_arg = modal.gpu.A100(
            memory=int(os.environ.get("MODAL_A100_MEMORY", "40"))
        )
    except TypeError:
        gpu_arg = modal.gpu.A100()
    ...
    if ModalStub is not None:
        stub = ModalStub("llm-controllability", image=image)
        stub_function = stub.function(**params)
        stub_cls = stub.cls(**params)
    else:
        app = ModalApp("llm-controllability")
...
def dream(c: DreamConfig, model=None, tokenizer=None):
    ...
    runner.minimize = bool(c.minimize)
